...
if \char`\"{}reverse\char`\"{} is False, the conformers in the residue which have the same number of protons will be fixed. if \char`\"{}reverse\char`\"{} is True, it's opposite, only the conformers with the same number of protons are not fixed. \begin{DoxyVerb}Check a head3.lst line to see if it's a dummy conformer.\end{DoxyVerb}
